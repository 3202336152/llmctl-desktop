...
CREATE TABLE `email_verification_codes` (
                                            `id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '验证码ID',
                                            ...
                                            `purpose` enum('REGISTER','LOGIN','RESET_PASSWORD','CHANGE_PASSWORD') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '验证码用途',
...
-- llmctl.providers_backup definition
...
CREATE TABLE `providers_backup` (
                                    ...
                                    `types` json DEFAULT NULL COMMENT 'Provider支持的CLI类型列表（JSON数组，claude code, codex, gemini, qoder）',
...
CREATE TABLE `users` (
                         ...
                         `display_name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL COMMENT '昵称',
...
CREATE TABLE `notifications` (
                                 ...
                                 `type` enum('SYSTEM','SESSION','STATISTICS','WARNING','SUCCESS','ERROR') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '通知类型',
                                 `title` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '通知标题',
                                 `content` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci COMMENT '通知内容（支持Markdown）',
                                 `data` json DEFAULT NULL COMMENT '额外数据（如会话ID、Provider ID等）',
                                 `is_read` tinyint(1) DEFAULT '0' COMMENT '是否已读',
                                 `priority` enum('LOW','NORMAL','HIGH','URGENT') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT 'NORMAL' COMMENT '优先级',
                                 `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                 `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                 `expires_at` timestamp NULL DEFAULT NULL COMMENT '过期时间',
                                 `action_url` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '点击后跳转的URL',
                                 `action_text` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '按钮文字',
                                 PRIMARY KEY (`id`),
                                 KEY `idx_user_id` (`user_id`),
                                 KEY `idx_type` (`type`),
...
) ENGINE=InnoDB AUTO_INCREMENT=39 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='通知消息表';
...
CREATE TABLE `sessions` (
                            ...
                            `type` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'CLI类型: claude code, codex, gemini, qoder',
                            ...
                            KEY `idx_user_id` (`user_id`),
                            KEY `idx_sessions_provider_id` (`provider_id`),
...
CREATE TABLE `tokens` (
                          ...
                          UNIQUE KEY `uk_user_token_hash` (`user_id`,`value_hash`),
                          KEY `idx_encryption_version` (`encryption_version`) COMMENT '加密版本索引',
                          KEY `idx_user_id` (`user_id`),
                          KEY `idx_tokens_provider_id` (`provider_id`),
                          CONSTRAINT `fk_token_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Token管理表';


-- llmctl.provider_configs definition

CREATE TABLE `provider_configs` (
                                    `id` bigint NOT NULL AUTO_INCREMENT COMMENT '配置ID',
                                    `provider_id` varchar(50) COLLATE utf8mb4_general_ci NOT NULL COMMENT 'Provider ID（外键）',
                                    `cli_type` enum('claude code','codex','gemini','qoder') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT 'CLI 类型',
                                    `config_data` json NOT NULL COMMENT 'CLI 配置数据（JSON格式）',
                                    `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                    `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                    PRIMARY KEY (`id`),
