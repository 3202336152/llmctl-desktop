...
### 📊 控制台仪表盘
- **快速操作卡片** - 一键跳转到创建会话、配置 Provider、MCP 管理、查看通知
- **系统状态概览** - 实时显示活跃会话数、Token 健康度、Provider 统计、MCP 服务器状态
- **会话时长趋势图** - 折线图展示最近会话时长变化趋势，支持 7/30/90 天时间范围切换
- **Provider 使用统计** - 柱状图展示每个 Provider 的总会话数和活跃会话数，实时更新
- **最近会话列表** - 显示所有会话记录，支持快速打开终端、查看会话状态，可滚动浏览
- **最近活动日志** - 时间线展示系统操作记录（会话启动/终止、Provider 启用/禁用、Token 状态变更）
